import {
  CurrentRepairState,
  Equipment,
  HierarchySelection,
  OrganizationNode,
  PreventiveMaintenance,
  ReactiveMaintenance,
  ReactiveRepairWorkflow,
  SparePart,
  SparePartHierarchySelection,
  SparePartStock,
  User,
  WorkflowStage,
} from "../types";

/**
 * Given an org ID, return all descendant org IDs (inclusive).
 * E.g. passing a corps ID returns the corps + all divisions, brigades, and units under it.
 */
export function getDescendantOrgIds(orgId: string): string[] {
  const result: string[] = [orgId];
  const queue = [orgId];
  while (queue.length > 0) {
    const current = queue.shift()!;
    for (const node of mockOrganizations) {
      if (node.parentId === current) {
        result.push(node.id);
        queue.push(node.id);
      }
    }
  }
  return result;
}

/**
 * Get the most specific org ID from a hierarchy selection,
 * then return all unit-level descendant IDs under it.
 */
export function getRelevantOrgIds(hierarchy: HierarchySelection): string[] {
  const mostSpecificId =
    hierarchy.unitId ||
    hierarchy.brigadeId ||
    hierarchy.divisionId ||
    hierarchy.corpsId;
  const allDescendants = getDescendantOrgIds(mostSpecificId);
  // Return only unit-level IDs (since equipment belongs to units)
  return allDescendants.filter(
    (id) => mockOrganizations.find((o) => o.id === id)?.level === "unit",
  );
}

/**
 * Get the display name for an organization ID.
 */
export function getOrgName(orgId: string): string {
  return mockOrganizations.find((o) => o.id === orgId)?.name ?? orgId;
}

/**
 * Auto-compute the current workflow stage from which date fields are filled.
 * The furthest date present determines the stage.
 */
export function computeCurrentStage(record: CurrentRepairState): WorkflowStage {
  if (record.repairDone) return "repair-complete";
  if (
    record.handoverToVendor ||
    typeof record.serviceabilityState === "boolean" ||
    record.vendorPDC
  )
    return "po";
  if (record.soByEngrRgt) return "engineer-regiment";
  if (record.roByDivHQ) return "engineer-regiment";
  if (record.notingOrder) return "div-hq";
  if (record.woToEMEBN) return "eme-battalion";
  return "workshop";
}

/**
 * Compute elapsed days for a CRS record.
 * - For completed repairs: from woFwdByUnit to repairDone date
 * - For in-progress repairs: from woFwdByUnit to today
 */
export function getElapsedDays(record: CurrentRepairState): number {
  const start = new Date(record.woFwdByUnit);
  let end: Date;

  if (computeCurrentStage(record) === "repair-complete" && record.repairDone) {
    end = new Date(record.repairDone);
  } else {
    end = new Date();
  }

  const diffMs = end.getTime() - start.getTime();
  return Math.max(0, Math.floor(diffMs / (1000 * 60 * 60 * 24)));
}

export const mockOrganizations: OrganizationNode[] = [
  { id: "corps-1", name: "1", level: "corps" },
  { id: "corps-2", name: "2", level: "corps" },
  { id: "corps-3", name: "3", level: "corps" },

  { id: "div-1", name: "10", level: "division", parentId: "corps-1" },
  { id: "div-2", name: "11", level: "division", parentId: "corps-2" },
  { id: "div-3", name: "12", level: "division", parentId: "corps-3" },
  { id: "div-4", name: "13", level: "division", parentId: "corps-3" },

  { id: "bde-1", name: "101", level: "brigade", parentId: "div-1" },
  { id: "bde-2", name: "102", level: "brigade", parentId: "div-1" },
  { id: "bde-3", name: "103", level: "brigade", parentId: "div-1" },
  { id: "bde-4", name: "10 AB", level: "brigade", parentId: "div-1" },
  { id: "bde-5", name: "DTU", level: "brigade", parentId: "div-1" },

  { id: "unit-1", name: "1 Madras", level: "unit", parentId: "bde-1" },
  { id: "unit-2", name: "2 Punjab", level: "unit", parentId: "bde-1" },
  { id: "unit-3", name: "3 Sikh", level: "unit", parentId: "bde-1" },

  { id: "unit-4", name: "1 Mahar", level: "unit", parentId: "bde-2" },
  { id: "unit-6", name: "2 JAK Rif", level: "unit", parentId: "bde-2" },
  { id: "unit-7", name: "3 JAK LI", level: "unit", parentId: "bde-2" },

  { id: "unit-8", name: "4 Madras", level: "unit", parentId: "bde-3" },
  { id: "unit-5", name: "5 Sikh", level: "unit", parentId: "bde-3" },
  { id: "unit-9", name: "6 Punjab", level: "unit", parentId: "bde-3" },

  { id: "unit-10", name: "120 Rajput", level: "unit", parentId: "bde-4" },
  { id: "unit-11", name: "121 Rajput", level: "unit", parentId: "bde-4" },
  { id: "unit-12", name: "122 Rajput", level: "unit", parentId: "bde-4" },

  { id: "unit-13", name: "Engineers", level: "unit", parentId: "bde-5" },
  { id: "unit-14", name: "Supply", level: "unit", parentId: "bde-5" },
  { id: "unit-15", name: "Signals", level: "unit", parentId: "bde-5" },
  { id: "unit-16", name: "Tankers", level: "unit", parentId: "bde-5" },
  { id: "unit-17", name: "Medical", level: "unit", parentId: "bde-5" },

  // ── Spare-parts parallel hierarchy: EME BN → Workshop → Unit ──
  {
    id: "eme-bn-1",
    name: "10 EME BN",
    level: "eme-battalion",
    parentId: "div-1",
  },

  {
    id: "ws-1",
    name: "Station Workshop",
    level: "workshop",
    parentId: "eme-bn-1",
  },
  {
    id: "ws-2",
    name: "Field Workshop",
    level: "workshop",
    parentId: "eme-bn-1",
  },

  // Units linked under workshops (spare parts context)
  {
    id: "sp-unit-1",
    name: "1 Madras",
    level: "unit",
    parentId: "ws-1",
  },
  {
    id: "sp-unit-2",
    name: "2 Punjab",
    level: "unit",
    parentId: "ws-1",
  },
  {
    id: "sp-unit-3",
    name: "3 Sikh",
    level: "unit",
    parentId: "ws-1",
  },
  {
    id: "sp-unit-4",
    name: "1 Mahar",
    level: "unit",
    parentId: "ws-1",
  },
  {
    id: "sp-unit-5",
    name: "5 Sikh",
    level: "unit",
    parentId: "ws-1",
  },
  {
    id: "sp-unit-6",
    name: "2 JAK Rif",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-7",
    name: "3 JAK LI",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-8",
    name: "4 Madras",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-9",
    name: "6 Punjab",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-10",
    name: "120 Rajput",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-11",
    name: "121 Rajput",
    level: "unit",
    parentId: "ws-2",
  },
  {
    id: "sp-unit-12",
    name: "122 Rajput",
    level: "unit",
    parentId: "ws-2",
  },
];

export const mockUsers: User[] = [
  { id: "u-admin", username: "admin", role: "admin" },
  {
    id: "u-unit",
    username: "unit.user",
    role: "unit",
    organizationId: "unit-1",
  },
  {
    id: "u-workshop",
    username: "workshop.user",
    role: "workshop",
    organizationId: "unit-1",
  },
  {
    id: "u-eme",
    username: "eme.user",
    role: "eme-battalion",
    organizationId: "unit-1",
  },
  {
    id: "u-div",
    username: "div.user",
    role: "div-hq",
    organizationId: "div-1",
  },
  {
    id: "u-engr",
    username: "engr.user",
    role: "engineer-regiment",
    organizationId: "bde-1",
  },
];

export const mockEquipment: Equipment[] = [
  {
    id: "eq-1",
    name: "Generator Set 5KVA",
    make: "Kirloskar",
    model: "OP-GEN-01",
    serialNumber: "GEN001",
    runningHoursLifetime: 12840,
    manufacturingYear: 2017,
    assetType: "generator",
    organizationId: "unit-1",
    cost: 185000,
  },
  {
    id: "eq-2",
    name: "Generator Set 10KVA",
    make: "Cummins",
    model: "C10D5",
    serialNumber: "GEN002",
    runningHoursLifetime: 9420,
    manufacturingYear: 2018,
    assetType: "generator",
    organizationId: "unit-2",
    cost: 350000,
  },
  {
    id: "eq-3",
    name: "WSS Pump 100HP",
    make: "KSB",
    model: "KP-100",
    serialNumber: "WSS001",
    assetType: "wss-pumps",
    organizationId: "unit-5",
    cost: 275000,
  },
  {
    id: "eq-4",
    name: "WSS Pump 50HP",
    make: "CRI",
    model: "CRI-50",
    serialNumber: "WSS002",
    assetType: "wss-pumps",
    organizationId: "unit-10",
    cost: 145000,
  },
  {
    id: "eq-5",
    name: "Generator Set 15KVA",
    make: "Ashok Leyland",
    model: "OP-GEN-02",
    serialNumber: "GEN003",
    runningHoursLifetime: 15480,
    manufacturingYear: 2016,
    assetType: "generator",
    organizationId: "unit-1",
    cost: 480000,
  },
  {
    id: "eq-6",
    name: "Generator Set 25KVA",
    make: "Mahindra Powerol",
    model: "MP-25",
    serialNumber: "GEN004",
    runningHoursLifetime: 11350,
    manufacturingYear: 2019,
    assetType: "generator",
    organizationId: "unit-6",
    cost: 625000,
  },
  {
    id: "eq-7",
    name: "WSS Pump 75HP",
    make: "Kirloskar",
    model: "KP-75",
    serialNumber: "WSS003",
    assetType: "wss-pumps",
    organizationId: "unit-7",
    cost: 210000,
  },
  {
    id: "eq-8",
    name: "Generator Set 7.5KVA",
    make: "Greaves",
    model: "GR-7.5",
    serialNumber: "GEN005",
    runningHoursLifetime: 8760,
    manufacturingYear: 2020,
    assetType: "generator",
    organizationId: "unit-8",
    cost: 240000,
  },
  {
    id: "eq-9",
    name: "Generator Set 20KVA",
    make: "Cummins",
    model: "C20D5",
    serialNumber: "GEN006",
    runningHoursLifetime: 16200,
    manufacturingYear: 2015,
    assetType: "generator",
    organizationId: "unit-11",
    cost: 550000,
  },
  {
    id: "eq-10",
    name: "WSS Pump 40HP",
    make: "Texmo",
    model: "TX-40",
    serialNumber: "WSS004",
    assetType: "wss-pumps",
    organizationId: "unit-14",
    cost: 120000,
  },
  {
    id: "eq-11",
    name: "WSS Pump 30HP",
    make: "CRI",
    model: "CRI-30",
    serialNumber: "WSS005",
    assetType: "wss-pumps",
    organizationId: "unit-3",
    cost: 95000,
  },
  {
    id: "eq-12",
    name: "Generator Set 12.5KVA",
    make: "Kirloskar",
    model: "KOEL-12.5",
    serialNumber: "GEN007",
    runningHoursLifetime: 10440,
    manufacturingYear: 2018,
    assetType: "generator",
    organizationId: "unit-13",
    cost: 420000,
  },
  {
    id: "eq-13",
    name: "Generator Set 30KVA",
    make: "Cummins",
    model: "OP-GEN-03",
    serialNumber: "GEN008",
    runningHoursLifetime: 18890,
    manufacturingYear: 2014,
    assetType: "generator",
    organizationId: "unit-1",
    cost: 750000,
  },
  {
    id: "eq-14",
    name: "WSS Pump 60HP",
    make: "KSB",
    model: "KP-60",
    serialNumber: "WSS006",
    assetType: "wss-pumps",
    organizationId: "unit-10",
    cost: 175000,
  },
  {
    id: "eq-15",
    name: "Generator Set 18KVA",
    make: "Mahindra Powerol",
    model: "MP-18",
    serialNumber: "GEN009",
    runningHoursLifetime: 12110,
    manufacturingYear: 2019,
    assetType: "generator",
    organizationId: "unit-6",
    cost: 510000,
  },
  {
    id: "eq-16",
    name: "Generator Set 8KVA",
    make: "Kirloskar",
    model: "KOEL-8KVA",
    serialNumber: "GEN010",
    runningHoursLifetime: 7985,
    manufacturingYear: 2021,
    assetType: "generator",
    organizationId: "unit-4",
    cost: 265000,
  },
  {
    id: "eq-17",
    name: "WSS Pump 20HP",
    make: "Texmo",
    model: "TX-20",
    serialNumber: "WSS007",
    assetType: "wss-pumps",
    organizationId: "unit-9",
    cost: 88000,
  },
  {
    id: "eq-18",
    name: "Generator Set 22KVA",
    make: "Cummins",
    model: "C22D5",
    serialNumber: "GEN011",
    runningHoursLifetime: 13670,
    manufacturingYear: 2017,
    assetType: "generator",
    organizationId: "unit-12",
    cost: 590000,
  },
  {
    id: "eq-19",
    name: "WSS Pump 15HP",
    make: "CRI",
    model: "CRI-15",
    serialNumber: "WSS008",
    assetType: "wss-pumps",
    organizationId: "unit-15",
    cost: 72000,
  },
  {
    id: "eq-20",
    name: "Generator Set 35KVA",
    make: "Ashok Leyland",
    model: "AL-35KVA",
    serialNumber: "GEN012",
    runningHoursLifetime: 17430,
    manufacturingYear: 2016,
    assetType: "generator",
    organizationId: "unit-16",
    cost: 820000,
  },
  {
    id: "eq-21",
    name: "WSS Pump 25HP",
    make: "KSB",
    model: "KP-25",
    serialNumber: "WSS009",
    assetType: "wss-pumps",
    organizationId: "unit-17",
    cost: 105000,
  },
];

export const mockPreventiveMaintenance: PreventiveMaintenance[] = [
  {
    id: "pm-1",
    organizationId: "unit-1",
    equipment: mockEquipment[0],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-28",
        completedDate: "2025-01-15",
        comment: "Engine oil and filter replaced",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-31",
        completedDate: "2025-04-20",
        comment: "Load test completed successfully",
      },
      { quarter: "Q3", dueDate: "2025-08-31", comment: "Unit in Fd ex" },
      { quarter: "Q4", dueDate: "2026-04-15" },
    ],
  },
  {
    id: "pm-2",
    organizationId: "unit-2",
    equipment: mockEquipment[1],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-15",
        completedDate: "2025-01-20",
        comment: "Battery terminals serviced",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-15",
        comment: "Trained Manpower Not-avlb",
      },
      { quarter: "Q3", dueDate: "2026-05-15" },
      { quarter: "Q4", dueDate: "2026-08-30" },
    ],
  },
  {
    id: "pm-3",
    organizationId: "unit-5",
    equipment: mockEquipment[2],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-31",
        completedDate: "2025-02-02",
        comment: "Alignment and vibration check done",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-30",
        completedDate: "2025-05-04",
        comment: "Seal inspection completed",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-30",
        completedDate: "2025-08-06",
        comment: "Bearing lubrication completed",
      },
      { quarter: "Q4", dueDate: "2026-03-31" },
    ],
  },
  {
    id: "pm-4",
    organizationId: "unit-10",
    equipment: mockEquipment[3],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-15",
        completedDate: "2025-01-28",
        comment: "Impeller and casing inspection done",
      },
      { quarter: "Q2", dueDate: "2026-04-30" },
      {
        quarter: "Q3",
        dueDate: "2025-09-15",
        comment: "Trained Manpower Not-avlb",
      },
      { quarter: "Q4", dueDate: "2026-07-31" },
    ],
  },
  {
    id: "pm-5",
    organizationId: "unit-6",
    equipment: mockEquipment[5],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-20",
        completedDate: "2025-02-12",
        comment: "Fuel line and injector cleaned",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-20",
        completedDate: "2025-05-16",
        comment: "Cooling fan belt replaced",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-20",
        comment: "Eqpt under Op commitment",
      },
      { quarter: "Q4", dueDate: "2026-05-20" },
    ],
  },
  {
    id: "pm-6",
    organizationId: "unit-7",
    equipment: mockEquipment[6],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-10",
        completedDate: "2025-01-30",
        comment: "Pump performance checked",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-10",
        comment: "Trained Manpower Not-avlb",
      },
      { quarter: "Q3", dueDate: "2026-06-10" },
      { quarter: "Q4", dueDate: "2026-09-10" },
    ],
  },
  {
    id: "pm-7",
    organizationId: "unit-8",
    equipment: mockEquipment[7],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-20",
        completedDate: "2025-01-18",
        comment: "AVR settings calibrated",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-20",
        completedDate: "2025-04-22",
        comment: "Routine workshop PM completed",
      },
      {
        quarter: "Q3",
        dueDate: "2025-08-20",
        completedDate: "2025-07-19",
        comment: "Cooling and load checks done",
      },
      { quarter: "Q4", dueDate: "2026-04-20" },
    ],
  },
  {
    id: "pm-8",
    organizationId: "unit-11",
    equipment: mockEquipment[8],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-05",
        completedDate: "2025-02-06",
        comment: "Starter assembly inspected",
      },
      { quarter: "Q2", dueDate: "2026-05-05" },
      {
        quarter: "Q3",
        dueDate: "2025-09-05",
        comment: "Spare parts shortage",
      },
      { quarter: "Q4", dueDate: "2026-08-05" },
    ],
  },
  {
    id: "pm-9",
    organizationId: "unit-14",
    equipment: mockEquipment[9],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-25",
        completedDate: "2025-03-02",
        comment: "Impeller balancing done",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-25",
        completedDate: "2025-06-01",
        comment: "Mechanical seal inspected",
      },
      { quarter: "Q3", dueDate: "2026-03-25" },
      { quarter: "Q4", dueDate: "2026-06-25" },
    ],
  },
  {
    id: "pm-10",
    organizationId: "unit-13",
    equipment: mockEquipment[11],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-15",
        completedDate: "2025-02-14",
        comment: "DG alignment and insulation test done",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-15",
        completedDate: "2025-05-18",
        comment: "AVR calibration completed",
      },
      { quarter: "Q3", dueDate: "2026-03-15" },
      { quarter: "Q4", dueDate: "2026-06-15" },
    ],
  },
  {
    id: "pm-11",
    organizationId: "unit-1",
    equipment: mockEquipment[12],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-25",
        completedDate: "2025-01-25",
        comment: "Fuel injector tuning completed",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-25",
        completedDate: "2025-04-28",
        comment: "Alternator brushes replaced",
      },
      { quarter: "Q3", dueDate: "2026-04-25" },
      { quarter: "Q4", dueDate: "2026-07-25" },
    ],
  },
  {
    id: "pm-12",
    organizationId: "unit-10",
    equipment: mockEquipment[13],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-10",
        completedDate: "2025-02-10",
        comment: "Impeller clearance adjusted",
      },
      { quarter: "Q2", dueDate: "2026-04-10" },
      { quarter: "Q3", dueDate: "2026-07-10" },
      { quarter: "Q4", dueDate: "2026-10-10" },
    ],
  },
  {
    id: "pm-13",
    organizationId: "unit-6",
    equipment: mockEquipment[14],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-20",
        completedDate: "2025-03-01",
        comment: "Air filter and oil replaced",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-20",
        completedDate: "2025-06-02",
        comment: "Battery load test passed",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-20",
        comment: "Eqpt access restd due to Ops",
      },
      { quarter: "Q4", dueDate: "2026-05-20" },
    ],
  },
  {
    id: "pm-14",
    organizationId: "unit-1",
    equipment: mockEquipment[4],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-28",
        completedDate: "2025-01-22",
        comment: "Governor calibration completed",
      },
      { quarter: "Q2", dueDate: "2025-05-28", comment: "Unit in Fd ex" },
      {
        quarter: "Q3",
        dueDate: "2025-08-28",
        comment: "Eqpt access restd due to Ops",
      },
      { quarter: "Q4", dueDate: "2026-04-28" },
    ],
  },
  {
    id: "pm-15",
    organizationId: "unit-3",
    equipment: mockEquipment[10],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-18",
        completedDate: "2025-02-18",
        comment: "Motor winding resistance checked",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-18",
        completedDate: "2025-05-25",
        comment: "Bearing replacement done",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-18",
        comment: "Spare parts shortage",
      },
      { quarter: "Q4", dueDate: "2026-05-18" },
    ],
  },
  {
    id: "pm-16",
    organizationId: "unit-4",
    equipment: mockEquipment[15],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-10",
        comment: "Spare parts shortage",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-10",
        comment: "Eqpt access restd due to Ops",
      },
      { quarter: "Q3", dueDate: "2026-04-10" },
      { quarter: "Q4", dueDate: "2026-07-10" },
    ],
  },
  {
    id: "pm-17",
    organizationId: "unit-9",
    equipment: mockEquipment[16],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-28",
        completedDate: "2025-03-10",
        comment: "Suction strainer cleaned",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-28",
        comment: "Trained Manpower Not-avlb",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-28",
        comment: "Trained Manpower Not-avlb",
      },
      { quarter: "Q4", dueDate: "2026-05-28" },
    ],
  },
  {
    id: "pm-18",
    organizationId: "unit-12",
    equipment: mockEquipment[17],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-28",
        completedDate: "2025-01-30",
        comment: "Load bank test completed",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-15",
        completedDate: "2025-04-15",
        comment: "AVR inspection done",
      },
      { quarter: "Q3", dueDate: "2025-08-15", comment: "Unit in Fd ex" },
      { quarter: "Q4", dueDate: "2026-04-15" },
    ],
  },
  {
    id: "pm-19",
    organizationId: "unit-15",
    equipment: mockEquipment[18],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-02-15",
        comment: "Trained Manpower Not-avlb",
      },
      {
        quarter: "Q2",
        dueDate: "2025-05-15",
        comment: "Trained Manpower Not-avlb",
      },
      { quarter: "Q3", dueDate: "2026-03-15" },
      { quarter: "Q4", dueDate: "2026-06-15" },
    ],
  },
  {
    id: "pm-20",
    organizationId: "unit-16",
    equipment: mockEquipment[19],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-08",
        completedDate: "2025-02-08",
        comment: "Fuel system flushed",
      },
      // {
      //   quarter: "Q2",
      //   dueDate: "2025-06-08",
      //   comment: "Equipment access rested due to ops",
      // },
      {
        quarter: "Q3",
        dueDate: "2025-09-08",
        comment: "Spare parts shortage",
      },
      { quarter: "Q4", dueDate: "2026-05-08" },
    ],
  },
  {
    id: "pm-21",
    organizationId: "unit-17",
    equipment: mockEquipment[20],
    quarters: [
      {
        quarter: "Q1",
        dueDate: "2025-03-30",
        completedDate: "2025-03-05",
        comment: "Valve seat inspection done",
      },
      {
        quarter: "Q2",
        dueDate: "2025-06-30",
        completedDate: "2025-06-12",
        comment: "Pressure test passed",
      },
      {
        quarter: "Q3",
        dueDate: "2025-09-30",
        comment: "Eqpt access restd due to Ops",
      },
      {
        quarter: "Q4",
        dueDate: "2025-12-30",
        comment: "Eqpt access restd due to Ops",
      },
    ],
  },
];

export const mockReactiveMaintenance: ReactiveMaintenance[] = [
  {
    id: "rm-1",
    organizationId: "unit-1",
    equipment: mockEquipment[0],
    repairs: [
      {
        id: "r-1",
        date: "2025-02-10",
        description: "Fuel pump replacement",
        cost: 18000,
        downtime: 23,
      },
      {
        id: "r-2",
        date: "2025-05-15",
        description: "Alternator rewinding and repair",
        cost: 32000,
        downtime: 25,
      },
      {
        id: "r-3",
        date: "2025-08-20",
        description: "Engine block crack repair and gasket replacement",
        cost: 45000,
        downtime: 28,
      },
      {
        id: "r-3a",
        date: "2025-10-05",
        description: "Turbocharger failure and replacement",
        cost: 52000,
        downtime: 27,
      },
      {
        id: "r-3b",
        date: "2025-12-18",
        description: "Complete cooling system overhaul",
        cost: 28000,
        downtime: 14,
      },
      {
        id: "r-3c",
        date: "2026-01-22",
        description: "Crankshaft bearing replacement",
        cost: 22000,
        downtime: 26,
      },
    ],
    totalBreakdowns: 6,
    totalCost: 197000,
    totalDowntime: 33,
  },
  {
    id: "rm-2",
    organizationId: "unit-2",
    equipment: mockEquipment[1],
    repairs: [
      {
        id: "r-4",
        date: "2025-03-12",
        description: "Battery replacement",
        cost: 2500,
        downtime: 21,
      },
      {
        id: "r-5",
        date: "2025-06-25",
        description: "Starter motor repair",
        cost: 6000,
        downtime: 22,
      },
      {
        id: "r-6",
        date: "2025-09-10",
        description: "Cooling system repair",
        cost: 4500,
        downtime: 12,
      },
    ],
    totalBreakdowns: 3,
    totalCost: 13000,
    totalDowntime: 5,
  },
  {
    id: "rm-3",
    organizationId: "unit-5",
    equipment: mockEquipment[2],
    repairs: [
      {
        id: "r-7",
        date: "2025-04-11",
        description: "Coupling replacement",
        cost: 7500,
        downtime: 33,
      },
      {
        id: "r-8",
        date: "2025-10-02",
        description: "Seal and bearing kit overhaul",
        cost: 9200,
        downtime: 24,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 16700,
    totalDowntime: 7,
  },
  {
    id: "rm-4",
    organizationId: "unit-6",
    equipment: mockEquipment[5],
    repairs: [
      {
        id: "r-9",
        date: "2025-02-21",
        description: "Injector calibration",
        cost: 4200,
        downtime: 22,
      },
      {
        id: "r-10",
        date: "2025-07-11",
        description: "Radiator leak repair",
        cost: 6100,
        downtime: 23,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 10300,
    totalDowntime: 5,
  },
  {
    id: "rm-5",
    organizationId: "unit-7",
    equipment: mockEquipment[6],
    repairs: [
      {
        id: "r-11",
        date: "2025-01-15",
        description: "Bearing replacement",
        cost: 12000,
        downtime: 23,
      },
      {
        id: "r-12",
        date: "2025-04-08",
        description: "Motor rewinding",
        cost: 35000,
        downtime: 27,
      },
      {
        id: "r-12a",
        date: "2025-06-20",
        description: "Impeller erosion repair and balancing",
        cost: 28000,
        downtime: 25,
      },
      {
        id: "r-12b",
        date: "2025-09-12",
        description: "Shaft sleeve replacement",
        cost: 42000,
        downtime: 26,
      },
      {
        id: "r-12c",
        date: "2025-11-28",
        description: "Discharge valve overhaul",
        cost: 18000,
        downtime: 23,
      },
      {
        id: "r-12d",
        date: "2026-01-10",
        description: "Complete mechanical seal assembly replacement",
        cost: 48000,
        downtime: 38,
      },
      {
        id: "r-12e",
        date: "2026-02-15",
        description: "Motor stator insulation failure",
        cost: 38000,
        downtime: 16,
      },
    ],
    totalBreakdowns: 7,
    totalCost: 221000,
    totalDowntime: 38,
  },
  {
    id: "rm-6",
    organizationId: "unit-11",
    equipment: mockEquipment[8],
    repairs: [
      {
        id: "r-13",
        date: "2025-01-27",
        description: "Control panel relay replacement",
        cost: 2800,
        downtime: 13,
      },
      {
        id: "r-14",
        date: "2025-09-04",
        description: "Starter motor overhaul",
        cost: 7600,
        downtime: 23,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 10400,
    totalDowntime: 4,
  },
  {
    id: "rm-7",
    organizationId: "unit-14",
    equipment: mockEquipment[9],
    repairs: [
      {
        id: "r-15",
        date: "2025-05-06",
        description: "Mechanical seal replacement",
        cost: 5400,
        downtime: 22,
      },
      {
        id: "r-16",
        date: "2025-11-02",
        description: "Shaft coupling realignment",
        cost: 4100,
        downtime: 21,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 9500,
    totalDowntime: 43,
  },
  {
    id: "rm-8",
    organizationId: "unit-1",
    equipment: mockEquipment[12],
    repairs: [
      {
        id: "r-17",
        date: "2025-04-03",
        description: "Governor calibration",
        cost: 6800,
        downtime: 22,
      },
      {
        id: "r-18",
        date: "2025-10-14",
        description: "Fuel pump seal replacement",
        cost: 3900,
        downtime: 21,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 10700,
    totalDowntime: 43,
  },
  {
    id: "rm-9",
    organizationId: "unit-10",
    equipment: mockEquipment[13],
    repairs: [
      {
        id: "r-19",
        date: "2025-05-22",
        description: "Mechanical seal replacement",
        cost: 5200,
        downtime: 22,
      },
      {
        id: "r-20",
        date: "2025-11-10",
        description: "Motor coupling correction",
        cost: 4400,
        downtime: 12,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 9600,
    totalDowntime: 34,
  },
  {
    id: "rm-10",
    organizationId: "unit-6",
    equipment: mockEquipment[14],
    repairs: [
      {
        id: "r-21",
        date: "2025-03-18",
        description: "Starter relay replacement",
        cost: 3100,
        downtime: 13,
      },
      {
        id: "r-22",
        date: "2025-09-29",
        description: "Radiator flush and hose change",
        cost: 5800,
        downtime: 23,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 8900,
    totalDowntime: 36,
  },
  {
    id: "rm-11",
    organizationId: "unit-3",
    equipment: mockEquipment[10],
    repairs: [
      {
        id: "r-23",
        date: "2025-02-14",
        description: "Motor winding burnout repair",
        cost: 22000,
        downtime: 25,
      },
      {
        id: "r-24",
        date: "2025-05-30",
        description: "Impeller replacement due to cavitation",
        cost: 18000,
        downtime: 14,
      },
      {
        id: "r-25",
        date: "2025-08-15",
        description: "Shaft seal assembly failure",
        cost: 15000,
        downtime: 13,
      },
      {
        id: "r-26",
        date: "2025-11-20",
        description: "Suction pipe corrosion replacement",
        cost: 25000,
        downtime: 26,
      },
      {
        id: "r-27",
        date: "2026-01-08",
        description: "Bearing housing crack repair",
        cost: 20000,
        downtime: 25,
      },
    ],
    totalBreakdowns: 5,
    totalCost: 100000,
    totalDowntime: 23,
  },
  {
    id: "rm-12",
    organizationId: "unit-10",
    equipment: mockEquipment[3],
    repairs: [
      {
        id: "r-28",
        date: "2025-03-05",
        description: "Control valve malfunction",
        cost: 8500,
        downtime: 22,
      },
      {
        id: "r-29",
        date: "2025-07-22",
        description: "Pressure gauge and safety valve replacement",
        cost: 6200,
        downtime: 21,
      },
      {
        id: "r-30",
        date: "2025-12-10",
        description: "Motor overload trip - starter assembly replaced",
        cost: 12000,
        downtime: 33,
      },
    ],
    totalBreakdowns: 3,
    totalCost: 26700,
    totalDowntime: 46,
  },
  {
    id: "rm-13",
    organizationId: "unit-4",
    equipment: mockEquipment[15],
    repairs: [
      {
        id: "r-31",
        date: "2025-04-12",
        description: "Exhaust manifold crack welding",
        cost: 14000,
        downtime: 33,
      },
      {
        id: "r-32",
        date: "2025-08-28",
        description: "Fuel injection pump overhaul",
        cost: 28000,
        downtime: 5,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 42000,
    totalDowntime: 8,
  },
  {
    id: "rm-14",
    organizationId: "unit-9",
    equipment: mockEquipment[16],
    repairs: [
      {
        id: "r-33",
        date: "2025-01-28",
        description: "Foot valve replacement",
        cost: 8000,
        downtime: 2,
      },
      {
        id: "r-34",
        date: "2025-04-15",
        description: "Motor phase failure - rewinding done",
        cost: 22000,
        downtime: 5,
      },
      {
        id: "r-35",
        date: "2025-07-10",
        description: "Delivery pipe burst replacement",
        cost: 16000,
        downtime: 3,
      },
      {
        id: "r-36",
        date: "2025-10-22",
        description: "Complete pump assembly overhaul",
        cost: 32000,
        downtime: 7,
      },
      {
        id: "r-37",
        date: "2026-01-15",
        description: "Non-return valve and strainer replacement",
        cost: 14000,
        downtime: 2,
      },
    ],
    totalBreakdowns: 5,
    totalCost: 92000,
    totalDowntime: 19,
  },
  {
    id: "rm-15",
    organizationId: "unit-15",
    equipment: mockEquipment[18],
    repairs: [
      {
        id: "r-38",
        date: "2025-02-20",
        description: "Motor bearing seizure",
        cost: 9500,
        downtime: 2,
      },
      {
        id: "r-39",
        date: "2025-05-18",
        description: "Impeller wear - replaced with new set",
        cost: 15000,
        downtime: 4,
      },
      {
        id: "r-40",
        date: "2025-08-30",
        description: "Motor starter contactor burnt",
        cost: 7500,
        downtime: 1,
      },
      {
        id: "r-41",
        date: "2025-11-14",
        description: "Shaft runout correction and seal kit",
        cost: 18000,
        downtime: 4,
      },
      {
        id: "r-42",
        date: "2026-02-02",
        description: "Complete pump casing replacement",
        cost: 28000,
        downtime: 5,
      },
    ],
    totalBreakdowns: 5,
    totalCost: 78000,
    totalDowntime: 16,
  },
  {
    id: "rm-16",
    organizationId: "unit-16",
    equipment: mockEquipment[19],
    repairs: [
      {
        id: "r-43",
        date: "2025-03-22",
        description: "Oil cooler line leak repair",
        cost: 9000,
        downtime: 22,
      },
      {
        id: "r-44",
        date: "2025-09-08",
        description: "AVR board failure replacement",
        cost: 35000,
        downtime: 24,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 44000,
    totalDowntime: 6,
  },
  {
    id: "rm-17",
    organizationId: "unit-1",
    equipment: mockEquipment[4],
    repairs: [
      {
        id: "r-45",
        date: "2025-02-14",
        description: "Cylinder head gasket blown - complete head overhaul",
        cost: 48000,
        downtime: 18,
      },
      {
        id: "r-46",
        date: "2025-06-10",
        description: "Turbocharger bearing failure and replacement",
        cost: 52000,
        downtime: 22,
      },
      {
        id: "r-47",
        date: "2025-10-18",
        description: "Crankshaft main bearing damage - engine rebuild",
        cost: 55000,
        downtime: 30,
      },
    ],
    totalBreakdowns: 3,
    totalCost: 155000,
    totalDowntime: 70,
  },
  {
    id: "rm-18",
    organizationId: "unit-13",
    equipment: mockEquipment[11],
    repairs: [
      {
        id: "r-48",
        date: "2025-03-20",
        description: "Alternator stator winding burnout - rewinding done",
        cost: 38000,
        downtime: 20,
      },
      {
        id: "r-49",
        date: "2025-07-05",
        description: "Engine oil pump failure and camshaft scoring repair",
        cost: 42000,
        downtime: 24,
      },
      {
        id: "r-50",
        date: "2025-11-12",
        description: "Radiator core replacement and coolant system overhaul",
        cost: 32000,
        downtime: 15,
      },
    ],
    totalBreakdowns: 3,
    totalCost: 112000,
    totalDowntime: 59,
  },
  {
    id: "rm-19",
    organizationId: "unit-12",
    equipment: mockEquipment[17],
    repairs: [
      {
        id: "r-51",
        date: "2025-04-08",
        description: "Fuel injection pump seizure - complete replacement",
        cost: 35000,
        downtime: 16,
      },
      {
        id: "r-52",
        date: "2025-08-22",
        description: "Governor actuator failure under load",
        cost: 28000,
        downtime: 12,
      },
    ],
    totalBreakdowns: 2,
    totalCost: 63000,
    totalDowntime: 28,
  },
];

export const mockReactiveRepairWorkflows: ReactiveRepairWorkflow[] = [
  {
    id: "rw-1",
    organizationId: "unit-1",
    equipment: mockEquipment[0],
    defect: "Engine overheating issue",
    createdByRole: "unit",
    currentStage: "div-hq",
    stageDates: {
      unitRaised: "2025-10-01",
      workshopReceived: "2025-10-02",
      emeEscalated: "2025-10-04",
      divRoApproved: "2025-10-07",
    },
    totalCost: 14000,
    totalDowntime: 9,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-10-01",
        remarks: "Defect raised with photos",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-10-02",
        remarks: "Initial diagnosis done",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-10-04",
        remarks: "Escalated to division for sanction",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-2",
    organizationId: "unit-2",
    equipment: mockEquipment[1],
    defect: "Voltage fluctuation and auto-cutoff",
    createdByRole: "unit",
    currentStage: "engineer-regiment",
    stageDates: {
      unitRaised: "2025-10-03",
      workshopReceived: "2025-10-04",
      emeEscalated: "2025-10-06",
      divRoApproved: "2025-10-09",
      engrSoIssued: "2025-10-12",
    },
    totalCost: 18000,
    totalDowntime: 12,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-10-03",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-10-04",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-10-06",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "completed",
        actionDate: "2025-10-09",
      },
      {
        stage: "engineer-regiment",
        role: "engineer-regiment",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-3",
    organizationId: "unit-5",
    equipment: mockEquipment[2],
    defect: "Pump shaft vibration beyond limit",
    createdByRole: "unit",
    currentStage: "repair-complete",
    stageDates: {
      unitRaised: "2025-09-01",
      workshopReceived: "2025-09-02",
      emeEscalated: "2025-09-04",
      divRoApproved: "2025-09-08",
      engrSoIssued: "2025-09-10",
      poRaised: "2025-09-13",
      repairCompleted: "2025-09-20",
    },
    totalCost: 22000,
    totalDowntime: 19,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-09-01",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-09-02",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-09-04",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "completed",
        actionDate: "2025-09-08",
      },
      {
        stage: "engineer-regiment",
        role: "engineer-regiment",
        status: "completed",
        actionDate: "2025-09-10",
      },
      {
        stage: "po",
        role: "admin",
        status: "completed",
        actionDate: "2025-09-13",
      },
      {
        stage: "repair-complete",
        role: "workshop",
        status: "completed",
        actionDate: "2025-09-20",
      },
    ],
  },
  {
    id: "rw-4",
    organizationId: "unit-6",
    equipment: mockEquipment[5],
    defect: "Radiator leakage and overheating under load",
    createdByRole: "unit",
    currentStage: "workshop",
    stageDates: {
      unitRaised: "2025-11-01",
      workshopReceived: "2025-11-02",
    },
    totalCost: 10300,
    totalDowntime: 5,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-11-01",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-5",
    organizationId: "unit-7",
    equipment: mockEquipment[6],
    defect: "Pump motor tripping at peak demand",
    createdByRole: "unit",
    currentStage: "eme-battalion",
    stageDates: {
      unitRaised: "2025-10-20",
      workshopReceived: "2025-10-21",
      emeEscalated: "2025-10-23",
    },
    totalCost: 13100,
    totalDowntime: 7,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-10-20",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-10-21",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-6",
    organizationId: "unit-11",
    equipment: mockEquipment[8],
    defect: "Starter motor failure during cold start",
    createdByRole: "unit",
    currentStage: "po",
    stageDates: {
      unitRaised: "2025-09-18",
      workshopReceived: "2025-09-19",
      emeEscalated: "2025-09-21",
      divRoApproved: "2025-09-24",
      engrSoIssued: "2025-09-26",
      poRaised: "2025-09-28",
    },
    totalCost: 10400,
    totalDowntime: 4,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-09-18",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-09-19",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-09-21",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "completed",
        actionDate: "2025-09-24",
      },
      {
        stage: "engineer-regiment",
        role: "engineer-regiment",
        status: "completed",
        actionDate: "2025-09-26",
      },
      {
        stage: "po",
        role: "admin",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-7",
    organizationId: "unit-14",
    equipment: mockEquipment[9],
    defect: "Frequent leakages from gland packing",
    createdByRole: "unit",
    currentStage: "repair-complete",
    stageDates: {
      unitRaised: "2025-08-05",
      workshopReceived: "2025-08-06",
      emeEscalated: "2025-08-08",
      divRoApproved: "2025-08-10",
      engrSoIssued: "2025-08-12",
      poRaised: "2025-08-14",
      repairCompleted: "2025-08-19",
    },
    totalCost: 9500,
    totalDowntime: 3,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-08-05",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-08-06",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-08-08",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "completed",
        actionDate: "2025-08-10",
      },
      {
        stage: "engineer-regiment",
        role: "engineer-regiment",
        status: "completed",
        actionDate: "2025-08-12",
      },
      {
        stage: "po",
        role: "admin",
        status: "completed",
        actionDate: "2025-08-14",
      },
      {
        stage: "repair-complete",
        role: "workshop",
        status: "completed",
        actionDate: "2025-08-19",
      },
    ],
  },
  {
    id: "rw-8",
    organizationId: "unit-1",
    equipment: mockEquipment[12],
    defect: "Governor hunting under variable load",
    createdByRole: "unit",
    currentStage: "div-hq",
    stageDates: {
      unitRaised: "2025-10-11",
      workshopReceived: "2025-10-12",
      emeEscalated: "2025-10-13",
      divRoApproved: "2025-10-16",
    },
    totalCost: 10700,
    totalDowntime: 3,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-10-11",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-10-12",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-10-13",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-9",
    organizationId: "unit-10",
    equipment: mockEquipment[13],
    defect: "Repeated shaft misalignment",
    createdByRole: "unit",
    currentStage: "engineer-regiment",
    stageDates: {
      unitRaised: "2025-11-04",
      workshopReceived: "2025-11-05",
      emeEscalated: "2025-11-07",
      divRoApproved: "2025-11-09",
      engrSoIssued: "2025-11-11",
    },
    totalCost: 9600,
    totalDowntime: 3,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-11-04",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "completed",
        actionDate: "2025-11-05",
      },
      {
        stage: "eme-battalion",
        role: "eme-battalion",
        status: "completed",
        actionDate: "2025-11-07",
      },
      {
        stage: "div-hq",
        role: "div-hq",
        status: "completed",
        actionDate: "2025-11-09",
      },
      {
        stage: "engineer-regiment",
        role: "engineer-regiment",
        status: "in-progress",
      },
    ],
  },
  {
    id: "rw-10",
    organizationId: "unit-6",
    equipment: mockEquipment[14],
    defect: "Frequent low-voltage trip",
    createdByRole: "unit",
    currentStage: "workshop",
    stageDates: {
      unitRaised: "2025-09-27",
      workshopReceived: "2025-09-28",
    },
    totalCost: 8900,
    totalDowntime: 3,
    history: [
      {
        stage: "unit",
        role: "unit",
        status: "completed",
        actionDate: "2025-09-27",
      },
      {
        stage: "workshop",
        role: "workshop",
        status: "in-progress",
      },
    ],
  },
];

export const mockCurrentRepairState: CurrentRepairState[] = [
  // â”€â”€ UNIT STAGE: WO just raised, not yet sent to workshop â”€â”€
  {
    id: "crs-1",
    organizationId: "unit-1",
    equipment: mockEquipment[0],
    defect: "Engine overheating under sustained load - coolant leak suspected",
    woFwdByUnit: "2026-02-22",
  },

  // â”€â”€ WORKSHOP STAGE: Unit forwarded WO, workshop inspecting â”€â”€
  {
    id: "crs-2",
    organizationId: "unit-6",
    equipment: mockEquipment[5],
    defect:
      "Radiator leakage and overheating under load - beyond workshop repair capability",
    woFwdByUnit: "2026-02-15",
  },
  {
    id: "crs-3",
    organizationId: "unit-4",
    equipment: mockEquipment[15],
    defect:
      "Alternator winding insulation failure - needs rewinding at base workshop",
    woFwdByUnit: "2026-02-18",
  },

  // â”€â”€ EME BATTALION STAGE: Workshop escalated, EME BN collating for noting â”€â”€
  {
    id: "crs-4",
    organizationId: "unit-7",
    equipment: mockEquipment[6],
    defect:
      "Pump motor tripping at peak demand - bearing race damage confirmed by workshop",
    woFwdByUnit: "2026-02-02",
    woToEMEBN: "2026-02-10",
  },
  {
    id: "crs-5",
    organizationId: "unit-8",
    equipment: mockEquipment[7],
    defect:
      "Control panel relay malfunction - repeated auto-shutdown under load",
    woFwdByUnit: "2026-02-05",
    woToEMEBN: "2026-02-13",
  },

  // â”€â”€ DIV HQ STAGE: Noting forwarded, awaiting Release Order â”€â”€
  {
    id: "crs-6",
    organizationId: "unit-2",
    equipment: mockEquipment[1],
    defect:
      "Voltage fluctuation and auto-cutoff - AVR module failure confirmed",
    woFwdByUnit: "2026-01-10",
    woToEMEBN: "2026-01-17",
    notingOrder: "2026-01-30",
  },
  {
    id: "crs-7",
    organizationId: "unit-1",
    equipment: mockEquipment[12],
    defect:
      "Governor hunting under variable load - electronic governor PCB defective",
    woFwdByUnit: "2026-01-20",
    woToEMEBN: "2026-01-28",
    notingOrder: "2026-02-08",
  },
  {
    id: "crs-8",
    organizationId: "unit-4",
    equipment: mockEquipment[3],
    defect:
      "Bearing seizure in main pump - impeller shaft damaged beyond field repair",
    woFwdByUnit: "2026-01-25",
    woToEMEBN: "2026-02-01",
    notingOrder: "2026-02-12",
  },

  // â”€â”€ ENGINEER REGIMENT STAGE: RO issued, awaiting Supply Order â”€â”€
  {
    id: "crs-9",
    organizationId: "unit-10",
    equipment: mockEquipment[13],
    defect:
      "Repeated shaft misalignment - coupling and foundation bolts worn out",
    woFwdByUnit: "2025-12-15",
    woToEMEBN: "2025-12-23",
    notingOrder: "2026-01-05",
    roByDivHQ: "2026-01-18",
  },
  {
    id: "crs-10",
    organizationId: "unit-3",
    equipment: mockEquipment[10],
    defect:
      "Exhaust manifold crack and gas leak - welding failed, OEM part required",
    woFwdByUnit: "2025-12-28",
    woToEMEBN: "2026-01-05",
    notingOrder: "2026-01-16",
    roByDivHQ: "2026-01-28",
  },

  // â”€â”€ PO / VENDOR STAGE: SO issued, equipment with vendor for repair â”€â”€
  {
    id: "crs-11",
    organizationId: "unit-11",
    equipment: mockEquipment[8],
    defect:
      "Starter motor failure - armature burnt, requires OEM replacement from Cummins",
    woFwdByUnit: "2025-11-20",
    woToEMEBN: "2025-11-28",
    notingOrder: "2025-12-10",
    roByDivHQ: "2025-12-22",
    soByEngrRgt: "2026-01-03",
    handoverToVendor: "2026-01-09",
    serviceabilityState: true,
    vendorName: "Cummins India Ltd",
    vendorPDC: "2026-03-15",
  },
  {
    id: "crs-12",
    organizationId: "unit-3",
    equipment: mockEquipment[3],
    defect:
      "Turbocharger oil seal failure - complete turbo assembly replacement needed",
    woFwdByUnit: "2025-12-01",
    woToEMEBN: "2025-12-05",
    notingOrder: "2025-12-10",
    roByDivHQ: "2025-12-15",
    soByEngrRgt: "2025-12-20",
    handoverToVendor: "2025-12-23",
    serviceabilityState: true,
    vendorName: "Turbo Energy Pvt Ltd",
    vendorPDC: "2026-03-30",
  },

  // â”€â”€ REPAIR COMPLETE: Full cycle completed â”€â”€
  {
    id: "crs-13",
    organizationId: "unit-5",
    equipment: mockEquipment[2],
    defect:
      "Pump shaft vibration beyond limit - shaft sleeve and bearings replaced by vendor",
    woFwdByUnit: "2025-08-10",
    woToEMEBN: "2025-08-16",
    notingOrder: "2025-08-24",
    roByDivHQ: "2025-09-01",
    soByEngrRgt: "2025-09-09",
    handoverToVendor: "2025-09-14",
    serviceabilityState: true,
    vendorName: "KSB Pumps Ltd",
    vendorPDC: "2025-11-10",
    repairDone: "2025-11-08",
  },
  {
    id: "crs-14",
    organizationId: "unit-14",
    equipment: mockEquipment[9],
    defect:
      "Frequent leakages from gland packing - mechanical seal replaced by vendor",
    woFwdByUnit: "2025-09-05",
    woToEMEBN: "2025-09-13",
    notingOrder: "2025-09-25",
    roByDivHQ: "2025-10-08",
    soByEngrRgt: "2025-10-18",
    handoverToVendor: "2025-10-30",
    serviceabilityState: false,
    vendorName: "Texmo Industries",
    vendorPDC: "2025-12-05",
    repairDone: "2025-11-28",
  },
  {
    id: "crs-15",
    organizationId: "unit-12",
    equipment: mockEquipment[17],
    defect:
      "Crankshaft end-play beyond tolerance - engine short block replaced at Cummins depot",
    woFwdByUnit: "2025-07-15",
    woToEMEBN: "2025-07-24",
    notingOrder: "2025-08-06",
    roByDivHQ: "2025-08-20",
    soByEngrRgt: "2025-09-01",
    handoverToVendor: "2025-09-05",
    serviceabilityState: true,
    vendorName: "Cummins India Ltd",
    vendorPDC: "2025-11-28",
    repairDone: "2025-11-22",
  },

  // â”€â”€ Additional: Long-stuck items for red flags â”€â”€
  {
    id: "crs-16",
    organizationId: "unit-6",
    equipment: mockEquipment[14],
    defect:
      "Frequent low-voltage trip - stator winding insulation breakdown, needs rewinding",
    woFwdByUnit: "2025-10-05",
    woToEMEBN: "2025-10-12",
    notingOrder: "2025-10-22",
    roByDivHQ: "2025-10-31",
    soByEngrRgt: "2025-11-08",
    handoverToVendor: "2025-11-14",
    serviceabilityState: true,
    vendorName: "Mahindra Powerol",
    vendorPDC: "2026-04-15",
  },
  {
    id: "crs-17",
    organizationId: "unit-9",
    equipment: mockEquipment[16],
    defect:
      "Submersible pump motor winding burnt - complete motor replacement required from OEM",
    woFwdByUnit: "2026-01-02",
    woToEMEBN: "2026-01-10",
    notingOrder: "2026-01-22",
  },
  {
    id: "crs-18",
    organizationId: "unit-15",
    equipment: mockEquipment[18],
    defect:
      "Impeller erosion causing low discharge pressure - OEM impeller out of stock",
    woFwdByUnit: "2025-11-10",
    woToEMEBN: "2025-11-19",
    notingOrder: "2025-12-02",
    roByDivHQ: "2025-12-16",
  },
  {
    id: "crs-19",
    organizationId: "unit-3",
    equipment: mockEquipment[10],
    defect:
      "Pump motor overheating due to damaged cooling fan assembly - OEM fan motor required",
    woFwdByUnit: "2025-06-10",
    woToEMEBN: "2025-06-15",
    notingOrder: "2025-06-21",
    roByDivHQ: "2025-06-28",
    soByEngrRgt: "2025-07-04",
    handoverToVendor: "2025-07-08",
    serviceabilityState: true,
    vendorName: "KSB Pumps Ltd",
    vendorPDC: "2025-09-30",
    repairDone: "2025-10-12",
  },
  {
    id: "crs-20",
    organizationId: "unit-4",
    equipment: mockEquipment[15],
    defect:
      "Generator control panel malfunction - PCB replacement needed from authorized dealer",
    woFwdByUnit: "2025-07-05",
    woToEMEBN: "2025-07-10",
    notingOrder: "2025-07-17",
    roByDivHQ: "2025-07-24",
    soByEngrRgt: "2025-07-31",
    handoverToVendor: "2025-08-04",
    serviceabilityState: true,
    vendorName: "Cummins India Ltd",
    vendorPDC: "2025-10-15",
    repairDone: "2025-10-15",
  },
  {
    id: "crs-21",
    organizationId: "unit-8",
    equipment: mockEquipment[7],
    defect:
      "Alternator bearing seized causing vibration - complete alternator overhaul required",
    woFwdByUnit: "2025-08-12",
    woToEMEBN: "2025-08-20",
    notingOrder: "2025-09-01",
    roByDivHQ: "2025-09-12",
    soByEngrRgt: "2025-09-22",
    handoverToVendor: "2025-09-26",
    serviceabilityState: true,
    vendorName: "Turbo Energy Pvt Ltd",
    vendorPDC: "2025-11-20",
    repairDone: "2025-12-08",
  },
  {
    id: "crs-22",
    organizationId: "unit-12",
    equipment: mockEquipment[17],
    defect:
      "Fuel injection pump timing fault - requires recalibration at authorized service center",
    woFwdByUnit: "2025-09-01",
    woToEMEBN: "2025-09-09",
    notingOrder: "2025-09-20",
    roByDivHQ: "2025-10-04",
    soByEngrRgt: "2025-10-15",
    handoverToVendor: "2025-10-19",
    serviceabilityState: true,
    vendorName: "Mahindra Powerol",
    vendorPDC: "2025-12-10",
    repairDone: "2026-01-05",
  },
  {
    id: "crs-23",
    organizationId: "unit-6",
    equipment: mockEquipment[14],
    defect:
      "Radiator core leak causing engine overheating - core replacement from OEM backorder",
    woFwdByUnit: "2025-10-20",
    woToEMEBN: "2025-10-26",
    notingOrder: "2025-11-03",
    roByDivHQ: "2025-11-12",
    soByEngrRgt: "2025-11-20",
    handoverToVendor: "2025-11-25",
    serviceabilityState: true,
    vendorName: "Cummins India Ltd",
    vendorPDC: "2026-02-15",
    repairDone: "2026-02-10",
  },
  {
    id: "crs-24",
    organizationId: "unit-13",
    equipment: mockEquipment[11],
    defect:
      "AVR module failure causing voltage fluctuation - imported module with long lead time",
    woFwdByUnit: "2025-11-05",
    woToEMEBN: "2025-11-14",
    notingOrder: "2025-11-26",
    roByDivHQ: "2025-12-10",
    soByEngrRgt: "2025-12-22",
    handoverToVendor: "2025-12-27",
    serviceabilityState: true,
    vendorName: "Kirloskar Electric",
    vendorPDC: "2026-02-28",
  },
  {
    id: "crs-25",
    organizationId: "unit-14",
    equipment: mockEquipment[9],
    defect:
      "Mechanical seal failure causing water ingress into motor - OEM seal set procurement",
    woFwdByUnit: "2025-05-15",
    woToEMEBN: "2025-05-22",
    notingOrder: "2025-06-04",
    roByDivHQ: "2025-06-18",
    soByEngrRgt: "2025-06-28",
    handoverToVendor: "2025-07-09",
    serviceabilityState: true,
    vendorName: "Texmo Industries",
    vendorPDC: "2025-08-20",
    repairDone: "2025-08-18",
  },
  {
    id: "crs-26",
    organizationId: "unit-7",
    equipment: mockEquipment[6],
    defect:
      "Centrifugal pump impeller cavitation damage - replacement impeller casting required",
    woFwdByUnit: "2025-12-01",
    woToEMEBN: "2025-12-08",
    notingOrder: "2025-12-17",
    roByDivHQ: "2025-12-27",
    soByEngrRgt: "2026-01-05",
    handoverToVendor: "2026-01-12",
    serviceabilityState: true,
    vendorName: "KSB Pumps Ltd",
    vendorPDC: "2026-03-20",
  },
  {
    id: "crs-27",
    organizationId: "unit-11",
    equipment: mockEquipment[8],
    defect:
      "Turbocharger wastegate actuator failure - OEM turbo assembly replacement required",
    woFwdByUnit: "2025-04-10",
    woToEMEBN: "2025-04-17",
    notingOrder: "2025-04-26",
    roByDivHQ: "2025-05-08",
    soByEngrRgt: "2025-05-19",
    handoverToVendor: "2025-05-26",
    serviceabilityState: true,
    vendorName: "Mahindra Powerol",
    vendorPDC: "2025-07-15",
    repairDone: "2025-08-10",
  },
  {
    id: "crs-28",
    organizationId: "unit-16",
    equipment: mockEquipment[19],
    defect:
      "Exhaust manifold crack causing gas leak - manifold casting not in stock at vendor",
    woFwdByUnit: "2025-03-05",
    woToEMEBN: "2025-03-12",
    notingOrder: "2025-03-24",
    roByDivHQ: "2025-04-08",
    soByEngrRgt: "2025-04-18",
    handoverToVendor: "2025-05-01",
    serviceabilityState: true,
    vendorName: "Mahindra Powerol",
    vendorPDC: "2025-06-10",
    repairDone: "2025-07-02",
  },
  {
    id: "crs-29",
    organizationId: "unit-2",
    equipment: mockEquipment[1],
    defect:
      "Starter motor solenoid burnt out - OEM solenoid assembly backordered from manufacturer",
    woFwdByUnit: "2025-05-20",
    woToEMEBN: "2025-05-23",
    notingOrder: "2025-05-27",
    roByDivHQ: "2025-05-31",
    soByEngrRgt: "2025-06-04",
    handoverToVendor: "2025-06-07",
    serviceabilityState: true,
    vendorName: "Turbo Energy Pvt Ltd",
    vendorPDC: "2025-08-25",
    repairDone: "2025-09-15",
  },
  {
    id: "crs-30",
    organizationId: "unit-17",
    equipment: mockEquipment[20],
    defect:
      "Gland packing failure causing excessive leakage - requires OEM packing set and shaft sleeve",
    woFwdByUnit: "2025-07-15",
    woToEMEBN: "2025-07-22",
    notingOrder: "2025-08-04",
    roByDivHQ: "2025-08-18",
    soByEngrRgt: "2025-08-28",
    handoverToVendor: "2025-09-13",
    serviceabilityState: true,
    vendorName: "Turbo Energy Pvt Ltd",
    vendorPDC: "2025-10-20",
    repairDone: "2025-11-08",
  },
  {
    id: "crs-31",
    organizationId: "unit-1",
    equipment: mockEquipment[4],
    defect:
      "Governor control module intermittent fault - replacement module awaited from OEM",
    woFwdByUnit: "2025-12-08",
    woToEMEBN: "2025-12-10",
    notingOrder: "2025-12-13",
    roByDivHQ: "2025-12-16",
    soByEngrRgt: "2025-12-18",
    handoverToVendor: "2025-12-20",
    serviceabilityState: true,
    vendorName: "Cummins India Ltd",
    vendorPDC: "2026-02-18",
  },
];

/* ══════════════════════════════════════════════════════════════
   Spare Parts – mock data & helpers
   ══════════════════════════════════════════════════════════════ */

/**
 * Get the most specific org ID from a spare-part hierarchy selection,
 * then return all unit-level descendant IDs under it (spare context).
 */
export function getSparePartOrgIds(
  hierarchy: SparePartHierarchySelection,
): string[] {
  const mostSpecificId =
    hierarchy.unitId ||
    hierarchy.workshopId ||
    hierarchy.emeBnId ||
    hierarchy.divisionId ||
    hierarchy.corpsId;
  const allDescendants = getDescendantOrgIds(mostSpecificId);
  return allDescendants.filter(
    (id) =>
      mockOrganizations.find((o) => o.id === id)?.level === "unit" &&
      id.startsWith("sp-unit"),
  );
}

export const mockSpareParts: SparePart[] = [
  // Generator parts
  {
    id: "sp-1",
    name: "Oil Filter",
    partNumber: "OF-CUM-2200",
    category: "Filters",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 850,
  },
  {
    id: "sp-2",
    name: "Fuel Filter",
    partNumber: "FF-CUM-3300",
    category: "Filters",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 1200,
  },
  {
    id: "sp-3",
    name: "Air Filter Element",
    partNumber: "AF-CUM-4400",
    category: "Filters",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 2400,
  },
  {
    id: "sp-4",
    name: "Drive Belt",
    partNumber: "DB-GEN-1100",
    category: "Belts & Hoses",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 1800,
  },
  {
    id: "sp-5",
    name: "Coolant Hose Assembly",
    partNumber: "CH-GEN-5500",
    category: "Belts & Hoses",
    assetType: "generator",
    unitOfMeasure: "Set",
    unitCost: 3200,
  },
  {
    id: "sp-6",
    name: "AVR Module",
    partNumber: "AVR-GEN-7700",
    category: "Electrical",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 15000,
  },
  {
    id: "sp-7",
    name: "Starter Motor Assembly",
    partNumber: "SM-CUM-8800",
    category: "Electrical",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 22000,
  },
  {
    id: "sp-8",
    name: "Radiator Core",
    partNumber: "RC-GEN-9900",
    category: "Cooling",
    assetType: "generator",
    unitOfMeasure: "Nos",
    unitCost: 28000,
  },
  // WSS Pump parts
  {
    id: "sp-9",
    name: "Mechanical Seal",
    partNumber: "MS-KSB-1010",
    category: "Seals & Gaskets",
    assetType: "wss-pumps",
    unitOfMeasure: "Nos",
    unitCost: 4500,
  },
  {
    id: "sp-10",
    name: "Impeller",
    partNumber: "IMP-KSB-2020",
    category: "Rotating Parts",
    assetType: "wss-pumps",
    unitOfMeasure: "Nos",
    unitCost: 18000,
  },
  {
    id: "sp-11",
    name: "Bearing Set (DE + NDE)",
    partNumber: "BR-KSB-3030",
    category: "Bearings",
    assetType: "wss-pumps",
    unitOfMeasure: "Set",
    unitCost: 6500,
  },
  {
    id: "sp-12",
    name: "Gland Packing",
    partNumber: "GP-PMP-4040",
    category: "Seals & Gaskets",
    assetType: "wss-pumps",
    unitOfMeasure: "Mtr",
    unitCost: 350,
  },
  {
    id: "sp-13",
    name: "Coupling Bush Set",
    partNumber: "CB-PMP-5050",
    category: "Coupling",
    assetType: "wss-pumps",
    unitOfMeasure: "Set",
    unitCost: 3800,
  },
  {
    id: "sp-14",
    name: "Motor Winding Coil",
    partNumber: "MW-PMP-6060",
    category: "Electrical",
    assetType: "wss-pumps",
    unitOfMeasure: "Set",
    unitCost: 12000,
  },
  {
    id: "sp-15",
    name: "Pressure Gauge",
    partNumber: "PG-PMP-7070",
    category: "Instruments",
    assetType: "wss-pumps",
    unitOfMeasure: "Nos",
    unitCost: 1600,
  },
  {
    id: "sp-16",
    name: "Non-Return Valve",
    partNumber: "NRV-PMP-8080",
    category: "Valves",
    assetType: "wss-pumps",
    unitOfMeasure: "Nos",
    unitCost: 8500,
  },
];

// Helper to build stock for every sp-unit × sparePart combination
function buildSparePartStock(): SparePartStock[] {
  const spUnits = mockOrganizations.filter(
    (o) => o.level === "unit" && o.id.startsWith("sp-unit"),
  );
  const stock: SparePartStock[] = [];
  let counter = 1;

  // Deterministic pseudo-random based on ids
  const hash = (a: string, b: string) => {
    let h = 0;
    const s = a + b;
    for (let i = 0; i < s.length; i++) {
      h = (h * 31 + s.charCodeAt(i)) | 0;
    }
    return Math.abs(h);
  };

  for (const unit of spUnits) {
    for (const part of mockSpareParts) {
      const h = hash(unit.id, part.id);
      const inStock = 1 + (h % 14); // 1-14

      // lastPurchaseDate — all items have a purchase date
      let lastPurchaseDate: string | null = null;
      if (inStock > 0) {
        const d = new Date("2026-03-14");
        // purchased 15-400 days ago
        const daysAgo = 15 + (h % 386);
        d.setDate(d.getDate() - daysAgo);
        lastPurchaseDate = d.toISOString().slice(0, 10);
      }

      // lastIssuedDate — ~15% are deadstock (not issued > 1 year)
      let lastIssuedDate: string | null = null;
      if (lastPurchaseDate) {
        const issueBucket = (h * 7 + 13) % 20; // different distribution
        const d = new Date("2026-03-14");
        if (issueBucket < 3) {
          // Deadstock (~15%): last issued 13-22 months ago
          const daysAgo = 400 + ((h * 3) % 270);
          d.setDate(d.getDate() - daysAgo);
        } else {
          // Recently issued (~85%): 3-150 days ago
          const daysAgo = 3 + ((h * 3) % 148);
          d.setDate(d.getDate() - daysAgo);
        }
        lastIssuedDate = d.toISOString().slice(0, 10);
      }

      stock.push({
        id: `sps-${counter++}`,
        sparePartId: part.id,
        organizationId: unit.id,
        inStockQty: inStock,
        lastPurchaseDate,
        lastIssuedDate,
      });
    }
  }
  return stock;
}

export const mockSparePartStock: SparePartStock[] = buildSparePartStock();
